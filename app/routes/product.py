...
def dashboard_summary(db: Session = Depends(get_db), user: str = Depends(get_current_user)):
    ...
    total_stock = db.query(Product).with_entities(Product.current_stock).all()
...
def low_stock_alerts(db: Session = Depends(get_db), user: str = Depends(get_current_user)):
    products = db.query(Product).filter(Product.current_stock <= Product.min_stock).all()
    return {"count": len(products), "products": products}


@router.get("/", response_model=list[ProductOut])
def get_products(db: Session = Depends(get_db), user: str = Depends(get_current_user)):
...
def stock_in(product_id: int, data: StockUpdate, db: Session = Depends(get_db), user: str = Depends(get_current_user)):
    product = db.query(Product).filter(Product.id == product_id).first()
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
...
def stock_out(product_id: int, data: StockUpdate, db: Session = Depends(get_db), user: str = Depends(get_current_user)):
    product = db.query(Product).filter(Product.id == product_id).first()
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
